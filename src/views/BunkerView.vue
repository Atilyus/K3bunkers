<template>
  <div class="bunker">
    ...
    <div class="container">
      ...
      <h4 style="text-align: center; padding-top:5px">{{title}}</h4>
      <div v-if="huco1">
        <hr>
      ...
      <br><div v-if="!seen" >
        ...
        <h6 class="form-check-label"> Toplam Kg.: {{topkg}}</h6>
      </div>
      <div v-if="seent">
...
export default {
  ...
  methods: {
    ...
    Getqu(){
      fetch(this.resturl+"get-tra/4").then(response => response.json())
    ...
    showt(){
      this.seen=false
      this.seent=true
      this.title="Robot Bunker Transfer"
      this.Getqu();
    },
    testqu(){
